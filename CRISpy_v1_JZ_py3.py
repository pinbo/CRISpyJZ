...
# command line parameters 1-4: 1, reference library file (fasta file with all references), 2, project ID, 3, ref_seq name, 4, ref_seq name (+ or - strand) 
# 5, seq_start; 6, seq_end, 7, guide RNA sequence (or its reverse complement if on - strand); 8, fastq_files
def get_parameters():
...
def search_fastq(ID,ref_seq,seq_start,seq_end,fastq_files,test_list):
    ...
    for k in test_dict:        #Insert the % values for test_dic at the end
        pd_columns.append(str('%'+k))
    # print "master_distance_and_count_summary"
    # print(master_distance_and_count_summary)
    csvout = open(str(save_dir+ID)+'.csv', "w")
    csvout.write(','.join(pd_columns) + "\n")
    for dd in master_distance_and_count_summary:
        rr = [str(dd[i]) if i in dd else "" for i in pd_columns]
        csvout.write(','.join(rr) + "\n")
    csvout.close()
